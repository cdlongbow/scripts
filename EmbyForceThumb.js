...
(function () {
  ...
  function cloneDefaultLibraries() {
    return [
      { id: '565027', name: 'wm', mode: 'thumb', policy: 'session', enabled: true },
      { id: '565031', name: 'gc', mode: 'thumb', policy: 'session', enabled: true },
      { id: '565033', name: 'om', mode: 'thumb', policy: 'session', enabled: true }
    ];
  }
  ...
  function patchUserSettings(userSettings) {
    ...
    userSettings.get = function (key, enableOnServer) {
      ...
      if (!rule || rule.policy === 'free' || !isControlledKey(key)) {
  ...
  function patch() {
    ...
    try {
      ...
      req(['libraryBrowser'], patchLibraryBrowser);
    } catch (e) {
      log('patch failed', e);
    }
  }
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '#' + PANEL_ID + ' .evm-dialog{width:min(920px,96vw);max-height:86vh;overflow:auto;background:#202020;color:#eee;border-radius:8px;box-shadow:0 18px 70px rgba(0,0,0,.55);}',
      ...
      '#' + PANEL_ID + ' .evm-row{display:grid;grid-template-columns:28px minmax(96px,.75fr) minmax(82px,.65fr) minmax(132px,1fr) minmax(230px,1.7fr) 36px;gap:8px;align-items:center;padding:10px;background:#2a2a2a;border:1px solid rgba(255,255,255,.08);border-radius:8px;}',
  ...
  function buildPanelHtml() {
    return [
      ...
      buildLibraryRows(),
      '</div>',
      '<div class="evm-footer"><div class="evm-help"><div>锁定：媒体库按“默认显示”设置打开，前台不能改成别的视图</div><div>临时：媒体库按“默认显示”设置打开, 可以临时切换，重启浏览器后恢复脚本的设置</div><div>解锁：脚本不再参与显示控制, 交由Emby与浏览器设置控制</div></div><button type="button" class="evm-primary" data-action="save">保存设置</button></div>',
      '</div>',
      '</div>'
    ].join('');
  }
  ...
  function bindPanel(panel) {
    ...
    panel.querySelector('[data-action="add-empty"]').addEventListener('click', function () {
      readPanel(panel);
      config.libraries.push(normalizeLibrary({ id: '', name: '', mode: 'thumb', policy: 'session', enabled: true }));
      rerenderPanel();
    });
