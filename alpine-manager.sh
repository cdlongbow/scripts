...
fix_repositories() {
    ...
    # 检查是否缺少必要的仓库
    if ! grep -q "main" /etc/apk/repositories 2>/dev/null || ! grep -q "community" /etc/apk/repositories 2>/dev/null; then
...
EOF
    ...
    echo ""
    print_info "当前仓库配置:"
    cat /etc/apk/repositories | grep -v "^#" | while read line; do
        echo "  ✓ $line"
    done
    echo ""
}
...
show_network_status() {
    ...
    echo "【网卡信息】"
    ip addr show | grep -E "^[0-9]+:|inet " | while read line; do echo "  $line"; done
    echo ""
    echo "【主网卡: $MAIN_IFACE】"
    echo "  IP地址: $CURRENT_IP"
    echo ""
    echo "【路由信息】"
    ip route | while read line; do echo "  $line"; done
    echo ""
    echo "【DNS 配置】"
    if [ -f /etc/resolv.conf ]; then
        cat /etc/resolv.conf | grep nameserver | while read line; do echo "  $line"; done
    else
        echo "  未配置 DNS"
    fi
    echo ""
...
configure_dhcp() {
    ...
    read -p "确认设置为 DHCP？(y/n): " -n 1 -r; echo
    [[ ! $REPLY =~ ^[Yy]$ ]] && { print_info "取消"; read -p "按回车键返回..."; return; }
...
configure_static() {
    ...
    read -p "静态IP地址 [例: 192.168.1.100]: " STATIC_IP
    while [ -z "$STATIC_IP" ]; do read -p "IP不能为空: " STATIC_IP; done
    read -p "子网掩码 [默认: 255.255.255.0]: " NETMASK
    NETMASK=${NETMASK:-255.255.255.0}
    read -p "网关地址: " GATEWAY
    while [ -z "$GATEWAY" ]; do read -p "网关不能为空: " GATEWAY; done
...
configure_dns() {
    ...
    case $dns_choice in
        ...
        2)
            ...
            # 设置第一个 DNS
            read -p "请输入首选 DNS [例: 8.8.8.8]: " dns1
            if [ -n "$dns1" ]; then
                ...
                # 询问是否设置第二个 DNS
                echo ""
                read -p "是否设置第二个 DNS？(y/n): " -n 1 -r
                echo
                if [[ $REPLY =~ ^[Yy]$ ]]; then
                    read -p "请输入备用 DNS [例: 8.8.4.4]: " dns2
                    if [ -n "$dns2" ]; then
...
install_docker() {
    ...
    # 设置开机自启
    if rc-update add docker boot 2>/dev/null; then
        print_info "已添加 docker 到开机启动"
    fi
    ...
    if docker info &>/dev/null; then
        ...
        docker version
        echo ""
        read -p "测试运行 hello-world？(y/n): " -n 1 -r; echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    while true; do
        ...
        case $choice in
            ...
            6)
                ...
                read -p "确认卸载 Docker？(y/n): " -n 1 -r; echo
                if [[ $REPLY =~ ^[Yy]$ ]]; then
                    service docker stop 2>/dev/null
                    pkill dockerd 2>/dev/null
                    rc-update del docker boot 2>/dev/null
...
install_dpanel() {
    ...
    if ! command -v docker &>/dev/null || ! docker info &>/dev/null; then
        ...
        echo ""
        read -p "是否先安装 Docker？(y/n): " -n 1 -r; echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            install_docker
            # 安装后再检查一次
            if ! docker info &>/dev/null; then
    ...
    echo ""
    read -p "继续安装 DPanel？(y/n): " -n 1 -r; echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        read -p "按回车键返回..."
        return
    fi
    ...
    # 执行 DPanel 安装
    curl -sSL https://dpanel.cc/quick.sh -o /tmp/quick.sh
    bash /tmp/quick.sh
    rm -f /tmp/quick.sh
...
install_all() {
    ...
    echo ""
    echo "=== 步骤 2/4: DNS 配置 ==="
    read -p "是否配置 DNS？(y/n): " -n 1 -r; echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        configure_dns
    fi
    ...
    read -p "是否安装 Docker？(y/n): " -n 1 -r; echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        install_docker
    else
    ...
    read -p "是否安装 DPanel？(y/n): " -n 1 -r; echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        install_dpanel
    fi
